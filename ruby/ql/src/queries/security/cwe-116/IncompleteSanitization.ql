...
predicate isSimple(RETV::RegExpTerm t) {
  t instanceof RETV::RegExpConstant
  or
  isSimple(t.(RETV::RegExpGroup).getAChild())
  or
...
predicate isSimpleCharacterClass(RETV::RegExpCharacterClass t) {
  not t.isInverted() and
  forall(RETV::RegExpTerm ch | ch = t.getAChild() | isSimple(ch))
}

/** Holds if `t` is an alternation of simple terms. */
predicate isSimpleAlt(RETV::RegExpAlt t) {
  forall(RETV::RegExpTerm ch | ch = t.getAChild() | isSimple(ch))
}
...
predicate allBackslashesEscaped(DataFlow::Node node) {
  exists(StringSubstitutionCall sub | node = sub |
    ...
    (
      ...
      exists(RETV::RegExpTerm root | root = sub.getPatternRegExp().getRegExpTerm() |
        not isSimple(root)
      )
    )
  )
...
/**
 * Holds if `sub` looks like a call `String#[g]sub[!]` that deliberately removes
 * the first occurrence of `str`.
 */
predicate removesFirstOccurence(StringSubstitutionCall sub, string str) {
  not exists(sub.getPatternRegExp()) and sub.replaces(str, "")
}
 ...
 */
predicate isDelimiterUnwrapper(DataFlow::Node leftUnwrap, DataFlow::Node rightUnwrap) {
